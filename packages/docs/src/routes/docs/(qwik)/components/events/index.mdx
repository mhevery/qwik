...
If we want to reuse the same event handler for multiple elements or event, we must extract the event handler into a [`QRLs`](/docs/advanced/qrl/) and pass it to the event listener. In order to do so, we need to import `$` from `@builder.io/qwik` and wrap the event handler in it.
